...
def delete():
    ...
    #delete a record
    c.execute("DELETE from addresses WHERE oid = " + DeleteId.get())
    print('Deleted Successfully')
    deleteId =messagebox.askquestion()
...
DeleteId = Entry(root, width=30)
DeleteId.grid(row=8, column=1)
DeleteId = Label(root, text ="Delete ID")
DeleteId.grid(row=8, column=0)

DeleteId_btn =Button(root, text="Delete")
DeleteId_btn.grid(row=9, column=0, columnspan=2,padx=10, pady=10, ipadx=120)

update = Button(root, text="update")
update.grid(row=10, column=0, columnspan=2,padx=10,pady=10,ipadx=100)
